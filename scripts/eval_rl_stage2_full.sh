#!/usr/bin/env bash
# Full post-train eval for RL Stage 2 v3 run covering BOTH epochs.
# Adapted from eval_rl_stage2_epoch1.sh — STEPS=(500 1000) now.
set -e
...
for step in "${STEPS[@]}"; do
    ...
    if [ ! -d "$MODEL_PATH" ]; then
        echo "[ERROR] Checkpoint not found: $MODEL_PATH (run merge_rl_ckpts.sh first)"
        exit 1
    fi
...
if [ ! -d outputs/ifeval_nothink/results/Qwen/Qwen3-14B ] 2>/dev/null; then
...
for step in "${STEPS[@]}"; do
    MODEL_PATH=${CKPT_ROOT}/global_step_${step}/hf_model
    IFEVAL_DONE_MARKER=$(find outputs/ifeval_nothink/results -path "*${RUN_TAG}/global_step_${step}*" -name "results_*.json" 2>/dev/null | head -1)
    if [ -n "$IFEVAL_DONE_MARKER" ]; then
        echo "[$(date)] IFEval step=${step} already done, skip."
        continue
    fi
